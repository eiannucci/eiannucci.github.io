...
export const NavMenu = () => {
  ...
  useEffect(() => {
    var gradients = ["linear-gradient(to right, #C8A2C8, #A692B7)", "linear-gradient(to right, #79aec1, #9B4F96)"];
  ...
  const navColorClass = classNames(styles['nav-color'], styles['nav-colorshift'], styles['nav-padding']);

  return (
    <Navbar expand="lg" className={`${navColorClass} fixed-top`} ref={navbarRef}>
      <Container fluid>
        ...
        <Navbar.Toggle aria-controls="basic-navbar-nav" ref={toggleRef} onClick={handleClick} />
        <Navbar.Collapse id="basic-navbar-nav" className={'nav-collapse'}>
          <Nav className="me-auto">
            ...
            <NavDropdown title="Animations & Interactive" id="animations_interactive">
              ...
              <NavDropdown.Item className={`${styles["nav-link"]}`} as={Link} to="/pg-cubewave" onClick={() => { handleLinkClick(); toggleRef.current.click(); }}>CubeWave</NavDropdown.Item>
              <NavDropdown.Item className={`${styles["nav-link"]}`} as={Link} to="/pg-insult-generator" onClick={() => { handleLinkClick(); toggleRef.current.click(); }}>Shakespearean Insult Generator</NavDropdown.Item>
              <NavDropdown.Item className={`${styles["nav-link"]}`} as={Link} to="/pg-animations" onClick={() => { handleLinkClick(); toggleRef.current.click(); }}>Animation Collection</NavDropdown.Item>
              <NavDropdown.Item className={`${styles["nav-link"]}`} as={Link} to="/pg-html-banners" onClick={() => { handleLinkClick(); toggleRef.current.click(); }}>HTML5 Banners</NavDropdown.Item>
            </NavDropdown>
            <NavDropdown title="Contact & Blog" id="contact_blog">
              <NavDropdown.Item href="mailto:elijah.iannucci@gmail.com?subject=Contact request from Elijah'a portfolio.">Email Me</NavDropdown.Item>
              <NavDropdown.Item href="https://www.linkedin.com/in/elijahiannucci/" target="_blank" rel="noopener noreferrer">Linkedin</NavDropdown.Item>
              <NavDropdown.Item href="https://elijahiannucci.blogspot.com" target="_blank" rel="noopener noreferrer">Blog</NavDropdown.Item>
            </NavDropdown>
          </Nav>
          <Nav>
            <FileDownload />
